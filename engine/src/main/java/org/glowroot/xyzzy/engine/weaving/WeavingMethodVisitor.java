...
import org.glowroot.xyzzy.instrumentation.api.internal.ParameterHolderImpl;
...
class WeavingMethodVisitor extends AdviceAdapter {
    ...
    private static final Method parameterHolderImplCreateMethod = new Method("create",
            "(Ljava/lang/Object;)Lorg/glowroot/xyzzy/instrumentation/api/internal/ParameterHolderImpl;");
    ...
    private void onMethodEnterInternal() {
        for (int i = 0; i < advisors.size(); i++) {
            ...
            if (advice.onAfterAdvice() != null || advice.onThrowAdvice() != null) {
                ...
                catchHandlers.add(ImmutableCatchHandler.builder()
                        .catchStartLabel(catchStartLabel)
                        .advisors(advisors.subList(0, i + 1))
                        .build());
            }
        }
        if (needsOnReturn) {
    ...
    private void loadOptionalReturnValue(int opcode, boolean dup) {
        if (opcode == RETURN) {
            // void
            visitMethodInsn(INVOKESTATIC, "org/glowroot/xyzzy/engine/bytecode/api/VoidReturn",
                    "getInstance", "()Lorg/glowroot/xyzzy/instrumentation/api/weaving/OptionalReturn;",
                    false);
        } else {
            loadReturnValue(opcode, dup, true);
            visitMethodInsn(INVOKESTATIC, "org/glowroot/xyzzy/engine/bytecode/api/NonVoidReturn", "create",
                    "(Ljava/lang/Object;)Lorg/glowroot/xyzzy/instrumentation/api/weaving/OptionalReturn;",
                    false);
        }
    }
