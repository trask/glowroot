...
glowroot.controller('ConfigEumCtrl', [
  '$scope',
  '$http',
  'confirmIfHasChanges',
  'httpErrors',
  function ($scope, $http, confirmIfHasChanges, httpErrors) {
    ...
    $scope.save = function (deferred) {
      ...
      $http.post('backend/config/eum?agent-id=' + encodeURIComponent($scope.agentId), postData)
    ...
    $http.get('backend/config/eum?agent-id=' + encodeURIComponent($scope.agentId))
        .then(function (response) {
          onNewData(response.data);
        }, function (response) {
