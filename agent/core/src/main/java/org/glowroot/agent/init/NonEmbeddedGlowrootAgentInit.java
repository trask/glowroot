    ...
    public void init(@Nullable File pluginsDir, final File confDir,
        ...
        OnEnteringMain onEnteringMain = new OnEnteringMain() {
            ...
            public void run() throws Exception {
                ...
                NettyWorkaround.run();
                Collector collector;
                Constructor<? extends Collector> delegatingCustomCollectorConstructor = null;
                if (customCollectorClass != null) {
                    try {
                        delegatingCustomCollectorConstructor =
                                customCollectorClass.getConstructor(Collector.class);
                    } catch (NoSuchMethodException e) {
                        logger.debug(e.getMessage(), e);
                    }
                }
                if (customCollectorClass != null && delegatingCustomCollectorConstructor == null) {
                    ...
                    if (delegatingCustomCollectorConstructor == null) {
                        collector = centralCollector;
                    } else {
                        startupLogger.info("using delegating collector: {}",
                                delegatingCustomCollectorConstructor.getName());
                        collector =
                                delegatingCustomCollectorConstructor.newInstance(centralCollector);
                    }
                }
