...
public class AnalyzedClassPlanBeeWithMoreBadPreloadCacheIT {
    ...
    public static void setUp() throws Exception {
        ...
        // put backwards parent/child relationship into preload-some-super-types-cache to ensure
        // that LinkageError is not thrown
        Files.write(
                AnalyzedClassPlanBeeWithMoreBadPreloadCacheIT.class.getName() + "$Y,"
                        + AnalyzedClassPlanBeeWithMoreBadPreloadCacheIT.class.getName() + "$Z",
                cacheFile, UTF_8);
    ...
    private static void updateCustomInstrumentationConfigs() throws Exception {
        CustomInstrumentationConfig config = CustomInstrumentationConfig.newBuilder()
                .setClassName(AnalyzedClassPlanBeeWithMoreBadPreloadCacheIT.class.getName() + "$Y")
                .setMethodName("y")
                .setMethodReturnType("")
    ...
    public static class ShouldNotLogWarningInAnalyzedWorldPlanB implements AppUnderTest {
        ...
        public void executeApp() throws Exception {
            Class.forName(AnalyzedClassPlanBeeWithMoreBadPreloadCacheIT.class.getName() + "$Z",
                    true, new DelegatingClassLoader());
        }
    }
