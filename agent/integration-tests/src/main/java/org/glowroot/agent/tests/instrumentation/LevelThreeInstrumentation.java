...
public class LevelThreeInstrumentation {
    ...
    public static class LevelThreeAdvice {
        ...
        private static final TimerName timerName = Agent.getTimerName("level three");

        @OnBefore
        public static Span onBefore(ThreadContext context, @BindParameter(0) final String arg1,
                @BindParameter(1) final String arg2) {
            return context.startLocalSpan(new MessageSupplier() {
                @Override
                public Message get() {
