...
public class BeanDescriptorInstrumentation {
    ...
    public static class GetBeanClassAdvice {
        ...
        private static final TimerName timerName = Agent.getTimerName("get bean class");

        @OnBefore
        public static Span onBefore(OptionalThreadContext context,
                @BindClassMeta TestClassMeta meta) {
            return context.startLocalSpan(MessageSupplier.create(meta.classInfo.getName()),
                    timerName);
        }
        @OnAfter
    ...
    public static class GetCustomizerClassAdvice {
        ...
        private static final TimerName timerName = Agent.getTimerName("get customizer class");

        @OnBefore
        public static Span onBefore(OptionalThreadContext context,
                @BindMethodMeta TestMethodMeta meta) {
            return context.startLocalSpan(MessageSupplier.create(meta.methodInfo.getName()),
                    timerName);
        }
        @OnAfter
        public static void onAfter(@BindTraveler Span traceEntry) {
