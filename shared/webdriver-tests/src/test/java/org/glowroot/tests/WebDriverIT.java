...
public abstract class WebDriverIT {
    ...
    private static void resetAllCentralConfig() throws Exception {
        ...
        deleteAllGauges();
        deleteAllAlerts();
        deleteAllCustomInstrumentation();
        resetUsers();
        resetRoles();
    ...
    private static void deleteAllCustomInstrumentation() throws Exception {
        String content = httpGet("http://localhost:" + getUiPort()
                + "/backend/config/custom-instrumentation?agent-id=" + agentId);
        ArrayNode arrayNode = (ArrayNode) new ObjectMapper().readTree(content).get("configs");
        for (JsonNode jsonNode : arrayNode) {
            String version = jsonNode.get("version").asText();
            httpPost("http://localhost:" + getUiPort()
                    + "/backend/config/custom-instrumentation/remove?agent-id=" + agentId,
                    "{\"versions\":[\"" + version + "\"]}");
        }
    }
