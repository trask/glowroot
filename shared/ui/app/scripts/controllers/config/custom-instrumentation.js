...
glowroot.controller('ConfigCustomInstrumentationCtrl', [
  '$scope',
  '$location',
  '$http',
  '$rootScope',
  ...
  function ($scope, $location, $http, $rootScope, $timeout, confirmIfHasChanges, httpErrors, queryStrings, modals, instrumentationExport) {
    ...
    var NEW_CONFIG = {
      ...
      // (see custom-instrumentation-list.js)
      classAnnotation: '',
      subTypeRestriction: '',
    ...
    if (version) {
      $http.get('backend/config/custom-instrumentation?agent-id=' + encodeURIComponent($scope.agentId) + '&version=' + version)
      ...
      $http.get('backend/config/new-custom-instrumentation-check-agent-connected?agent-id=' + encodeURIComponent($scope.agentId))
          .then(function (response) {
            $scope.loaded = true;
    ...
    $scope.save = function (deferred) {
      ...
      var url;
      if (version) {
        url = 'backend/config/custom-instrumentation/update';
      } else {
        url = 'backend/config/custom-instrumentation/add';
      }
    ...
    $scope.delete = function (deferred) {
      ...
      var agentId = $scope.agentId;
      $http.post('backend/config/custom-instrumentation/remove?agent-id=' + encodeURIComponent(agentId), postData)
          .then(function () {
            removeConfirmIfHasChangesListener();
            if (agentId) {
              $location.url('config/custom-instrumentation-list?agent-id=' + encodeURIComponent(agentId)).replace();
            } else {
              $location.url('config/custom-instrumentation-list').replace();
            }
          }, function (response) {
            httpErrors.handle(response, deferred);
          });
