...
glowroot.controller('ConfigCustomInstrumentationListCtrl', [
  '$scope',
  '$location',
  '$http',
  '$timeout',
  ...
  function ($scope, $location, $http, $timeout, $q, locationChanges, queryStrings, httpErrors, modals, instrumentationExport) {
    ...
    $scope.customInstrumentationQueryString = function (config) {
      var query = {};
      if ($scope.agentId) {
    ...
    function refresh(deferred) {
      $http.get('backend/config/custom-instrumentation?agent-id=' + encodeURIComponent($scope.agentId))
    ...
    $scope.showApplyChangesButton = function () {
      return $scope.jvmRetransformClassesSupported && $scope.agentRollup.permissions.config.edit.instrumentation;
    };
    ...
    $scope.deleteAll = function () {
      ...
      $http.post('backend/config/custom-instrumentation/remove?agent-id=' + encodeURIComponent($scope.agentId), postData)
          .then(function () {
    ...
    $scope.importFromJson = function () {
      ...
      $http.post('backend/config/custom-instrumentation/import?agent-id=' + encodeURIComponent($scope.agentId), postData)
          .then(function () {
            var deferred = $q.defer();
            deferred.promise.finally(function () {
              // leave spinner going until subsequent refresh is complete
