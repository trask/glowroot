...
glowroot.controller('ConfigInstrumentationCtrl', [
  ...
  function ($scope, $stateParams, $http, $location, confirmIfHasChanges, httpErrors, queryStrings) {
    ...
    $scope.$watch('config.properties', function (newValue) {
    ...
    $scope.save = function (deferred) {
      ...
      $http.post('backend/config/instrumentation?agent-id=' + encodeURIComponent($scope.agentId), postData)
    ...
    $http.get('backend/config/instrumentation' + queryStrings.encodeObject(queryData))
        .then(function (response) {
          onNewData(response.data);
        }, function (response) {
