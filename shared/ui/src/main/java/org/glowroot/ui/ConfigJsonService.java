...
class ConfigJsonService {
    ...
    String getInstrumentationConfig(@BindAgentId String agentId,
        ...
        Optional<String> instrumentationId = request.id();
        if (instrumentationId.isPresent()) {
            return getInstrumentationConfigInternal(agentId, request.id().get());
        } else {
            List<InstrumentationResponse> responses = Lists.newArrayList();
            for (InstrumentationConfig config : configRepository
                    .getInstrumentationConfigs(agentId)) {
                responses.add(ImmutableInstrumentationResponse.builder()
                        .id(config.getId())
                        .name(config.getName())
                        .hasConfig(config.getPropertyCount() > 0)
                        .build());
            }
    ...
    @POST(path = "/backend/config/instrumentation",
            permission = "agent:config:edit:instrumentation")
    String updateInstrumentationModuleConfig(@BindAgentId String agentId,
            @BindRequest InstrumentationModuleUpdateRequest request) throws Exception {
        String id = request.id();
        if (id.equals("jdbc")) {
            String firstInvalidRegularExpression =
                    getFirstInvalidJdbcInstrumentationRegularExpressions(request.properties());
        ...
        InstrumentationConfig.Builder builder = InstrumentationConfig.newBuilder()
                .setId(id);
        for (ModulePropertyDto prop : request.properties()) {
            builder.addProperty(prop.convert());
        }
        try {
            configRepository.updateInstrumentationConfig(agentId, builder.build(),
                    request.version());
        } catch (OptimisticLockException e) {
            throw new JsonServiceException(PRECONDITION_FAILED, e);
        }
        return getInstrumentationConfigInternal(agentId, id);
    }
    ...
    private String getInstrumentationConfigInternal(String agentId, String instrumentationId)
            throws Exception {
        InstrumentationConfig config =
                configRepository.getInstrumentationConfig(agentId, instrumentationId);
        if (config == null) {
            throw new IllegalArgumentException(
                    "Instrumentation id not found: " + instrumentationId);
        }
        return mapper.writeValueAsString(InstrumentationModuleConfigDto.create(config));
    }

    private static @Nullable String getFirstInvalidJdbcInstrumentationRegularExpressions(
            List<ImmutableModulePropertyDto> properties) {
        for (ModulePropertyDto property : properties) {
    ...
    @Value.Immutable
    interface InstrumentationModuleUpdateRequest {
        String id();
        List<ImmutableModulePropertyDto> properties();
        String version();
    }
    ...
    @Value.Immutable
    abstract static class InstrumentationModuleConfigDto {

        abstract String name();
        abstract List<ImmutableModulePropertyDto> properties();
        abstract String version();

        private static InstrumentationModuleConfigDto create(InstrumentationConfig config) {
            ImmutableInstrumentationModuleConfigDto.Builder builder =
                    ImmutableInstrumentationModuleConfigDto.builder()
                            .name(config.getName());
            for (InstrumentationProperty property : config.getPropertyList()) {
                builder.addProperties(ModulePropertyDto.create(property));
            }
    ...
    // only used in response
    @Value.Immutable
    abstract static class ModulePropertyDto {
        ...
        private InstrumentationProperty convert() {
            return InstrumentationProperty.newBuilder()
                    .setName(name())
                    .setValue(getValue())
                    .build();
        }

        private InstrumentationProperty.Value getValue() {
            Object value = value();
            switch (type()) {
                case BOOLEAN:
                    checkNotNull(value);
                    return InstrumentationProperty.Value.newBuilder().setBval((Boolean) value)
                            .build();
                case DOUBLE:
                    if (value == null) {
                        return InstrumentationProperty.Value.newBuilder().setDvalNull(true).build();
                    } else {
                        return InstrumentationProperty.Value.newBuilder()
                                .setDval(((Number) value).doubleValue()).build();
                    }
                case STRING:
                    checkNotNull(value);
                    return InstrumentationProperty.Value.newBuilder().setSval((String) value)
                            .build();
                case LIST:
                    checkNotNull(value);
                    ...
                    return InstrumentationProperty.Value.newBuilder().setLval(lval).build();
                default:
                    throw new IllegalStateException("Unexpected property type: " + type());
            }
        }

        private static ImmutableModulePropertyDto create(InstrumentationProperty property) {
            return ImmutableModulePropertyDto.builder()
        ...
        private static PropertyType getPropertyType(InstrumentationProperty.Value.ValCase valCase) {
            switch (valCase) {
                case BVAL:
        ...
        private static @Nullable Object getPropertyValue(InstrumentationProperty.Value value) {
            InstrumentationProperty.Value.ValCase valCase = value.getValCase();
            switch (valCase) {
                case BVAL:
