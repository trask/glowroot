...
//
// this tests that the servlet instrumentation doesn't try to capture the request parameters (which
// it does via getParameterMap()) after the call to getParameterValues() (if getParameterValues() is
// called from inside of getParameterMap()), since this would lock the parameter map causing the
// outer call to getParameterMap() to fail
